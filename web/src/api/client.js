...
export const api = {
  ...
  taskAiSummary: (id) => apiFetch(`/tasks/${id}/ai-summary`),
  taskAiAnalyze: (id) => apiFetch(`/tasks/${id}/ai-analyze`, { method: 'POST' }),
  ...
  myTaskTriggers:        () => apiFetch('/me/task-triggers'),
  ...
  hrmosMonthlyCheck:     (month, opts = {}) => apiFetch(`/admin/hrmos-monthly-check?${new URLSearchParams({ ...(month?{month}:{}), ...(opts.refresh?{refresh:'1'}:{}) }).toString()}`),
  hrmosExcludedList:     () => apiFetch('/admin/hrmos-excluded'),
  hrmosExcludeUser:      (body) => apiFetch('/admin/hrmos-excluded', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }),
  hrmosUnexcludeUser:    (id) => apiFetch(`/admin/hrmos-excluded/${id}`, { method: 'DELETE' }),
  ...
  crmDealPayments: (id) => crmFetch(`/deals/${id}/payments`),
  crmExpectedPayments: (month) => crmFetch(`/expected-payments${month?`?month=${month}`:''}`),
  crmGenerateExpected: (dealId, force=false) => crmFetch(`/deals/${dealId}/generate-expected`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ force }) }),
  crmExtendDeal: (dealId, months) => crmFetch(`/deals/${dealId}/extend`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ months }) }),
  ...
  mediaMasterUpdate:  (id, body) => apiFetch(`/media-master/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }),
  mediaMasterCreate:  (body) => apiFetch(`/media-master`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }),
  mediaMasterDelete:  (id) => apiFetch(`/media-master/${id}`, { method: 'DELETE' }),
